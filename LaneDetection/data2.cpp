...
void detect_line(Mat &left_img,vector<Vec4i> &left_lines, Mat &dst, Mat &imgResult, int distance)
{
  ...
  for(int i = 0; i < lines.size(); i++)
  {
    ...
    {
      real_lines_x[real_line_idx] = x_min;
      real_lines_y[real_line_idx++] = pre_y;
  ...
  {
    circle(imgResult,Point(real_lines_x[i]+distance,real_lines_y[i]),6,Scalar(0,0,255),CV_FILLED);
  }
  ...
  for( int iy=0 ; iy<real_line_idx ; iy++ )
    ...
    if(iy == 0)
    {
      dx1 = real_lines_x[iy]+distance;
      dy1 = data;
    }
    else if(iy == 1)
    {
      dx2 = real_lines_x[iy]+distance;
      dy2 = data;
    }
...
int main()
{
  ...
  src_vertices[0] = Point(src.cols/2-120,250);   // 좌상
  src_vertices[1] = Point(src.cols/2+120, 250);  // 우상
  src_vertices[2] = Point(src.cols/2+220, src.rows); // 우하
  src_vertices[3] = Point(src.cols/2-220, src.rows);
